...
fn main() {
    ...
    {
        let slope = bf_slope(&weight, &avg_k);
        let y_intercept = y_intercept(mean(&weight[..]), mean(&avg_k), slope);
        println!("Kinetic:\n weight: {:?} avg_k: {:?}\n slope: {} y-intercept: {}", &weight, &avg_k, slope, y_intercept);
    }
    {
        let slope = bf_slope(&weight, &avg_s);
        let y_intercept = y_intercept(mean(&weight[..]), mean(&avg_s), slope);
